...
{       std::lock_guard<std::mutex> lock(client_links_mutex_);
        ...
        threadPool->enqueue([this, ep_ev]() {  // lambda统一处理即可
                ...
                int recv_size = recv(ep_ev.data.fd, buf, BUFF_SIZE, 0);
...
void MasterServer::startHashslotService(MasterServer *m)
{
    ...
    while (1) {
        ...
        if (task.first == TASK_ADD) {
            ...
            // 2. notify all (excluding the new node) of the CHANGE of the hashslot         // SWAP 2 and 4
...
{           std::lock_guard<std::mutex> lock(m->hash_slot_mutex_);
            ...
            // first send to the new cache server the complete hashslot info
            char send_buff[BUFF_SIZE_LONG];
            bzero(send_buff, BUFF_SIZE_LONG);
            ...
            // then sequentially connect each old cache server and send hashslot change info
            int cur = 0;
...
{           std::lock_guard<std::mutex> lock(m->hash_slot_mutex_);
            ...
            // finally, sequentially connect each client and send the hashslot change info
...
{           std::lock_guard<std::mutex> lock(m->hash_slot_mutex_);
            ...
            // first sequentially connect each remaining cache server
            int cur = 0;
            for (auto cache_it = new_hash_slot->cbegin(); cache_it != new_hash_slot->cend(); ++cache_it) {
                char send_buff[BUFF_SIZE];
                bzero(send_buff, BUFF_SIZE);
...
{           std::lock_guard<std::mutex> lock(m->cache_links_mutex_);
            ...
            // MUST NOT immediately close(sockfd), use EPOLL instead
            epoll_event ev;
            bzero(&ev, sizeof(ev));
